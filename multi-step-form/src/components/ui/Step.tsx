...
const Step: React.FC<StepProps> = ({ stepNumber, stepDescription, selected }) => {
  return (
    <div className="w-full relative">
      <div className={`absolute w-[33px] h-[33px] font-ubuntu rounded-full flex justify-center items-center top-0 ${selected ? 'bg-cyan-100' : 'bg-none border border-cyan-100 text-white'}`}>{stepNumber}</div>
